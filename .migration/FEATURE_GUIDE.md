...
| `to_path(value)` | ✅ | Convert to Path object |
...
**Location:** `scriptman/cache/` (to be created)
...
| **Observer Integration** | | |
| Auto-log operations | 📋 | Log hits/misses via observer |
...
**Location:** `scriptman/types.py` (to be created)

Reusable type definitions and decorators for sync/async function handling.

| Feature | Status | Description |
|---------|--------|-------------|
| Type aliases | 📋 | `T`, `P`, `R`, `SyncFunc`, `AsyncFunc` |
| `wrap_sync_async()` | 📋 | Decorator helper for sync/async |
| `is_async()` | 📋 | Check if function is async |
| `BaseModelT` | 📋 | TypeVar for Pydantic models |
